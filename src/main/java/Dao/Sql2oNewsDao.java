...
public class Sql2oNewsDao implements NewsDao {
    ...
    public void add(News news) {
        String checkedType = confirmDepartments(news.getType());
    ...
    public void addNewsToDepartment(int deptid, int newsid, int userid) {
        ...
        try (Connection con = DB.sql2o.open()) {
            con.createQuery(sql).addParameter("deptid",deptid).addParameter("newsid",newsid).addParameter("userid",userid)
                    .executeUpdate();
        } catch (Sql2oException ex){
            System.out.println(ex);
        }
